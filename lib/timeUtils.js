...
/**
 * 判斷目前是否在氣象預報排程夜間靜音時段（台灣時間 00:00:00 ~ 06:59:59）。
 * 早上 07:00 起恢復查詢與預報。
 * @param {Date} [targetDate] 可選目標時間（預設當前時間）
 * @returns {boolean}
 */
export function isWeatherQuietHours(targetDate) {
  const hour = targetDate ? toTaiwanTime(targetDate).getUTCHours() : getTaiwanNow().hour;
