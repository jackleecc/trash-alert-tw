...
export async function getTodaySuspendedCities(dateStr) {
  ...
  // 快取命中判定 (FIX-1)：
  // 1. 若 fetched_at 存在，代表 DGPA 已實際抓取過並寫入快取。
  // 2. 若有明確記錄停收 (suspended_cities 非空或 is_suspended = true)，亦視為有效快取。
  // 3. 若僅由 truckApi 等其他模組寫入失敗計數（fetched_at 為 null 且無停收標記），視為快取缺失，必須向 DGPA 查詢。
  const isCacheValid =
    cached !== null &&
    (
      (cached.fetched_at !== null && cached.fetched_at !== undefined) ||
      (Array.isArray(cached.suspended_cities) && cached.suspended_cities.length > 0) ||
      cached.is_suspended === true
    );
