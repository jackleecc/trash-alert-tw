...
export async function checkUpcomingRain(lat, lng) {
  ...
  try {
    const headers = {
      'User-Agent':
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (compatible; TrashAlertBot/1.0; +https://github.com/jackleecc/trash-alert-tw)',
    };
    const [weatherRes, aqRes] = await Promise.all([
      fetch(weatherUrl, { headers, signal: controller.signal }),
      fetch(aqUrl, { headers, signal: controller.signal })
    ]);
    ...
    // 定義各項異常條件（降雨機率高於 60% 以上才發送通知）
    const willRain = nextHourProb >= 60;
    const uvWarning = nextHourUv >= 8; // UV >= 8 為危險/極危險
    const pmWarning = nextHourPm25 >= 70; // PM2.5 >= 70μg/m³ (提升門檻至 70 以上含 70)
    ...
    if (pmWarning) warnings.push(`😷 空氣品質不良 (PM2.5 濃度 ${nextHourPm25}μg/m³ ≥ 70)，建議配戴口罩防護😷`);
    ...
    if (err.name === 'AbortError') {
      console.error(`[WeatherAPI] 取得氣象預報逾時 (8s) (lat:${lat}, lng:${lng})`);
    } else {
...
/**
 * 格式化垃圾車到站時的出門天氣速報（針對黃昏/夜間清運時段，排除無實質意義之紫外線）
 * @param {{ prob?: number, precipitation?: number, pm25?: number, willRain?: boolean, pmWarning?: boolean, desc?: string } | null | undefined} weatherInfo
 * @returns {string} 天氣與空氣品質摘要字串，若氣象預報失敗則回傳空字串
 */
export function formatArrivalWeatherSummary(weatherInfo) {
  if (!weatherInfo || typeof weatherInfo !== 'object') return '';

  // 氣象 API 失敗或不完整時，不產出誤導性的假晴天資料
  if (
    weatherInfo.desc === '氣象預報讀取失敗' ||
    weatherInfo.desc === '無法取得完整的預報資料'
  ) {
    return '';
  }
  ...
  if (weatherInfo.willRain) {
    lines.push(`🌧️ 降雨機率：${weatherInfo.prob}% / 預估雨量：${weatherInfo.precipitation}mm（將有降雨，出門請攜帶雨具🌂）`);
  } else {
    lines.push(`☀️ 降雨機率：${weatherInfo.prob ?? 0}%（天氣良好，免帶雨具）`);
  }

  // 2. 空氣品質資訊
  if (weatherInfo.pmWarning) {
    lines.push(`😷 空氣品質：PM2.5 ${weatherInfo.pm25} μg/m³（不良，建議配戴口罩防護😷）`);
  } else {
    lines.push(`🍃 空氣品質：PM2.5 ${weatherInfo.pm25 ?? 0} μg/m³（良好）`);
  }
