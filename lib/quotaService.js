...
export async function syncLineConsumption(yearMonth = getYearMonth(), customFetch = null) {
 ...
 * @param {string} [yearMonth] - 格式 'YYYY-MM'，預設當前月份
 * @returns {Promise<{ month: string, usedCount: number, maxQuota: number, remaining: number, isMelted: boolean }>}
 */
export async function getQuotaSnapshot(yearMonth = getYearMonth()) {
  const quota = await getOrCreateQuotaRecord(yearMonth);
