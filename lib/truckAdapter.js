...
export function normalizeTruckRecord(raw) {
  ...
  if (rawTime) {
    if (typeof rawTime === 'object') {
      if (rawTime.year !== undefined && rawTime.month !== undefined && rawTime.date !== undefined) {
        const fullYear = 1900 + Number(rawTime.year);
        const m = String(Number(rawTime.month) + 1).padStart(2, '0');
        const d = String(rawTime.date).padStart(2, '0');
        const hh = String(rawTime.hours || 0).padStart(2, '0');
        const mm = String(rawTime.minutes || 0).padStart(2, '0');
        const ss = String(rawTime.seconds || 0).padStart(2, '0');
        time = `${fullYear}-${m}-${d} ${hh}:${mm}:${ss}`;
      } else if (rawTime.time) {
        time = new Date(rawTime.time).toISOString();
      }
    } else {
  ...
  // 6. 提取即時車速 (speed)
  const rawSpeed = getFirstAvailableValue(raw, ['speed', 'Speed', 'SPEED']);
  let speed;
  if (rawSpeed !== undefined && rawSpeed !== null && rawSpeed !== '') {
    const s = Number(rawSpeed);
