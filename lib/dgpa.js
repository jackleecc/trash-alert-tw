...
export function parseHtmlForKaohsiung(html) {
  ...
  // 3. 搜尋高雄市區塊 (例如 <tr> 或前後文字區間)，確保該區塊明確包含停班停課字樣
  // 匹配「高雄市」後方 300 字元內是否包含停班/停課
  const kaohsiungBlockMatch = html.match(/高雄市[\s\S]{1,300}?(停止上班|停止上課|全天停止|已達停止)/);
  if (kaohsiungBlockMatch) {
    // 檢查該鄰近區段是否明確標示為照常上班/上課
    const matchedSegment = kaohsiungBlockMatch[0];
    if (matchedSegment.includes('照常上班') && matchedSegment.includes('照常上課')) {
      return false;
    }
    return true;
  }
