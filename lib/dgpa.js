 ...
 */
export const MONITORED_CITIES = ['高雄市', '新北市', '桃園市'];

/**
 * 判斷 DGPA 頁面原始 HTML 中，指定縣市是否有停班停課公告。
 * @param {string} html
 * @param {string} cityName - 縣市名稱（例如 '高雄市'、'新北市'）
 * @returns {boolean}
 */
export function parseSuspensionForCity(html, cityName) {
  ...
  if (!html.includes(cityName)) {
    return false;
  }

  // 2. 排除常見的否定/排除語句 (例如「除高雄市外」)
  const excludePattern = new RegExp(`除\\s*${cityName}\\s*外`);
  ...
  const blockPattern = new RegExp(
    `${cityName}[\\s\\S]{1,100}?(停止上班|停止上課|全天停止|已達停止)`
  );
