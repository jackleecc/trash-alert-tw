...
export function isMovingTowardsApproach(
  ...
  // 邊際條件 1：車輛處於停靠或極低速作業中 (時速 < 5 km/h 或位移 < 10m)
  // GPS 靜止漂移時方位角會隨機震盪，此時車輛已在站點旁停滯，直接豁免放行（優先於方向判定）
  if (typeof truck.speed === 'number' && truck.speed < 5) {
    return true;
  }
...
/**
 * 統一縣市名稱異體字標準化（例如「臺南市」->「台南市」）
 * @param {string | null | undefined} city
 * @returns {string | null}
 */
export function normalizeCity(city) {
  if (!city || typeof city !== 'string') return null;
  return city.replace(/^臺/, '台').trim();
}

/**
 * 解析路線所屬縣市。優先讀取 routes.city 欄位，若尚未設定則自路線名稱/描述推斷。
 * 統一異體字標準化（如「臺南市」->「台南市」），確保系統全域標準一致。
 * @param {object} route
 * @returns {string | null}
 */
export function resolveRouteCity(route) {
  let city = route?.city || null;
  if (!city) {
    const text = `${route?.name || ''} ${route?.description || ''} ${route?.id || ''}`;
    if (text.includes('新北') || text.includes('汐止')) city = '新北市';
    else if (text.includes('桃園') || text.includes('中壢') || text.includes('八德') || text.includes('蘆竹') || text.includes('龜山') || text.includes('平鎮') || text.includes('楊梅')) city = '桃園市';
    else if (text.includes('高雄') || text.includes('路竹') || text.includes('新興') || text.includes('苓雅') || text.includes('三民')) city = '高雄市';
    else if (text.includes('台南') || text.includes('臺南') || text.includes('永康') || text.includes('安南') || text.includes('東區') || text.includes('北區') || text.includes('中西區') || text.includes('南區') || text.includes('安平')) city = '台南市';
  }
