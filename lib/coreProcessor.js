...
export async function processTruckArrivals(truckData, taiwanNowInfo, suspendedCities = []) {
  ...
  const { data: routes, error: routeErr } = await supabase
    .from('routes')
    .select('id, name, active_days, city')
    .eq('is_active', true);
  ...
  for (const r of routes) {
    ...
    // 若該路線所屬縣市正處於停班停課，則跳過
    if (r.city && suspendedCities.includes(r.city)) {
      console.log(`[Core] 路線 [${r.id}] ${r.name} 因 ${r.city} 天災停收，今日略過。`);
      continue;
    }
