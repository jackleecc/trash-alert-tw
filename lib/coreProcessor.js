...
export function resolveRouteCity(route) {
  ...
  if (text.includes('桃園') || text.includes('中壢') || text.includes('八德') || text.includes('蘆竹') || text.includes('龜山') || text.includes('平鎮') || text.includes('楊梅')) return '桃園市';
...
export async function getActiveSubscriptionContext(
  ...
  for (const stop of stops || []) {
    ...
    // 智慧縮時窗口判定：預設表定前 18 分鐘至後 25 分鐘
    const beforeMins = options.beforeMinutes ?? 18;
    const afterMins = options.afterMinutes ?? 25;
    ...
    if (city) {
      if (
        ...
        console.log(`[Core] 📱 臺南市已配置純手機中繼模式 (SKIP_TAINAN_POLLING)，略過雲端主動輪詢。`);
      } else {
