 ...
 * @param {number} [beforeMinutes=20] - 提前分鐘數（預設 20 分鐘）
 * @param {number} [afterMinutes=40] - 延後分鐘數（預設 40 分鐘）
 ...
 *
 * @param {Array<{ route_id: string, lat: number, lng: number, car_id?: string, waste_type?: string }>} truckData
 * @param {Array<{ id: number, route_id: string, name: string, lat: number, lng: number, schedule_time?: string }>} stops
...
export function findNearbyTruckArrivals(
  ...
  for (const truck of truckData || []) {
    ...
    for (const stop of stops || []) {
      ...
      // 班表時間窗過濾：若該站點有表定時間，且當前時間不在清運時間窗內，則略過
 ...
 */
export async function processTruckArrivals(truckData, taiwanNowInfo, suspendedCities = []) {
  ...
  // 2. 查詢所有啟用路線（使用 select('*') 避免缺少 city 欄位時錯誤）
  ...
  if (routeErr) {
    ...
    return { ok: false, matchedArrivals: 0, sentNotifications: 0, closestTrucks: [], reason: 'db-route-error' };
  }

  if (!routes || routes.length === 0) {
    return { ok: true, matchedArrivals: 0, sentNotifications: 0, closestTrucks: [], reason: 'no-active-routes' };
  }
  ...
  if (activeRouteIds.length === 0) {
    ...
    return { ok: true, matchedArrivals: 0, sentNotifications: 0, closestTrucks: [], reason: 'no-routes-today' };
  }
  ...
  if (stopErr) {
    ...
    return { ok: false, matchedArrivals: 0, sentNotifications: 0, closestTrucks: [], reason: 'db-stop-error' };
  }
  ...
  if (groupErr) {
    ...
    return { ok: false, matchedArrivals: 0, sentNotifications: 0, closestTrucks: [], reason: 'db-group-error' };
  }

  const activeGroupIds = (groups || []).map((group) => group.group_id);
  if (activeGroupIds.length === 0) {
    return { ok: true, matchedArrivals: 0, sentNotifications: 0, closestTrucks: [], reason: 'no-active-groups' };
  }
  ...
  if (subErr) {
    ...
    return { ok: false, matchedArrivals: 0, sentNotifications: 0, closestTrucks: [], reason: 'db-sub-error' };
  }
